...
async function bootstrap() {
    ...
    logger.log(`Found existing admin: ${existingAdmin ? existingAdmin.email : '❌ none'}`, 'SeedAdmin');
    ...
    if (!existingAdmin) {
        ...
        logger.log(`✅ Admin user created: ${admin.email} (status: ACTIVE, emailVerified: true)`, 'SeedAdmin');
    } else {
        ...
        if (existingAdmin.status !== UserStatus.ACTIVE || !existingAdmin.isEmailVerified) {
            ...
            logger.log(`✅ Existing admin updated: ${existingAdmin.email} (status: ACTIVE, emailVerified: true)`, 'SeedAdmin');
        } else {
