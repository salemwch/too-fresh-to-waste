...
export class ModerationLogController {
    ...
    @ApiQuery({ name: 'category', required: false, enum: LogCategory, description: 'Filter by log category' })
    ...
    @ApiQuery({ name: 'isAutomated', required: false, type: Boolean, description: 'Filter automated vs manual actions' })
    ...
    async getModerationLogs(
        @Query(ValidationPipe) queryDto: ModerationLogQueryDto
    ) {
        ...
        const filters: any = {
            ...
            targetId: queryDto.targetId,
            isAutomated: queryDto.isAutomated,
            tags: queryDto.tags
        };
    ...
    async getUserModerationHistory(
        ...
        const filteredHistory = userRole === UserRole.MODERATOR ?
            history.map(log => ({
                level: log.level,
    ...
    async getMyModerationActivity(
        ...
        const filters: any = { performedBy: userId };
