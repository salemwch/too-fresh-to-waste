...
export class ArchiveService {
  ...
  async archiveAllExpired(): Promise<ArchiveResult[]> {
    ...
    results.push(await this.archiveAndPurge(this.orderModel, standardQuery, 'orders'));
    results.push(await this.archiveAndPurge(this.reviewModel, standardQuery, 'reviews'));
    results.push(await this.archiveAndPurge(this.offerModel, standardQuery, 'offers'));
    results.push(
      await this.archiveAndPurge(this.establishmentModel, standardQuery, 'establishments'),
    );
    results.push(
      await this.archiveAndPurge(this.userDonationModel, standardQuery, 'userdonations'),
    );
    ...
    results.push(await this.archiveAndPurge(this.userModel, userQuery, 'users'));
  ...
  private async archiveAndPurge<T>(
    ...
    try {
      ...
      this.logger.log(`Found ${docs.length} expired soft-deleted records in ${sourceCollection}`);
      ...
      for (let i = 0; i < docs.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          this.logger.warn(
            `Skipped ${mongoErr?.writeErrors?.length ?? 0} duplicate archive entries in ${sourceCollection}`,
          );
        }

        // Hard-delete from source — deleteMany is NOT affected by
        // soft-delete /^find/ middleware.
        await model.deleteMany({ _id: { $in: ids } } as FilterQuery<T>);
        totalArchived += batch.length;
      }

      this.logger.log(`Archived and purged ${totalArchived} records from ${sourceCollection}`);
    } catch (error) {
      this.logger.error(
