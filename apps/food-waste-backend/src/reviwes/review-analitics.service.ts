...
export class ReviewAnalyticsService {
    ...
    async generateEstablishmentInsights(
        ...
        try {
            ...
            this.logger.error(
                `Failed to generate insights for establishment ${establishmentId}:`,
                error,
            );
            throw error;
        }
    ...
    private getTopPerformers(industryType: string, limit: number): Promise<any[]> {
    ...
    private async analyzeEstablishmentTrends(
        ...
        const ratingTrend = this.calculateTrend(recentData.averageRating, previousData.averageRating, 0.1);
        const reviewVolumeTrend = this.calculateTrend(recentData.reviewCount, previousData.reviewCount, 0.05);
    ...
    private calculateTrend(current: number, previous: number, threshold: number): 'up' | 'down' | 'stable' {
    ...
    private calculateSentimentTrend(currentPositive: number, previousPositive: number): 'improving' | 'declining' | 'stable' {
    ...
    private processSentimentArray(sentiments: string[]): { [key: string]: number } {
    ...
    private processRatingArray(ratings: number[]): { [key: string]: number } {
     ...
     */
    async generateIndustryReport(industryType: string, timeframe: number = 90): Promise<any> {
        try {
            const startDate = new Date(Date.now() - timeframe * 24 * 60 * 60 * 1000);
    ...
    async generateWeeklyReports(): Promise<void> {
        try {
            ...
            const industryTypes = ['restaurant', 'bakery', 'grocery_store', 'cafe', 'fast_food', 'supermarket'];
  ...
  private async analyzeSentimentTrends(
    ...
    const distribution = result.distribution.reduce((acc, item) => {
    ...
    const keywordAnalysis = result.keywords.map(item => ({
