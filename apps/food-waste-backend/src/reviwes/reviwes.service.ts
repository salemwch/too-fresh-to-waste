...
export class ReviewsService {
            ...
            await session.withTransaction(async () => {
                ...
                if (createReviewDto.offerId) {
                    const offer = await this.offerModel
                        .findById(createReviewDto.offerId)
                        .session(session);
                ...
                const reviewData = {
                    ...
                    orderId: createReviewDto.orderId ? new Types.ObjectId(createReviewDto.orderId) : undefined,
                    offerId: createReviewDto.offerId ? new Types.ObjectId(createReviewDto.offerId) : undefined,
                    isVerifiedPurchase,
                        ...
                        reviews: [
                            ...
                            { $unwind: '$reviewerId' },
                            {
                                $lookup: {
                                    ...
                                    as: 'establishmentId',
                                    pipeline: [
                                        { $project: { name: 1, type: 1, averageRating: 1, totalReviews: 1 } }
                                    ]
                                }
                            },
    ...
    async findOne(
        ...
        try {
            ...
            if (userId && userRole !== UserRole.ADMIN) {
                const canAccess = await this.checkReviewAccess(review, userId, userRole);
                if (!canAccess) {
    ...
    async getAnalytics(analyticsDto: ReviewAnalyticsDto): Promise<ReviewAnalytics> {
        try {
            ...
            const ratingDistribution = results.ratingDistribution.reduce((acc, item) => {
                acc[`rating_${item._id}`] = item.count;
                return acc;
            }, {});

            const sentimentDistribution = results.sentimentDistribution.reduce((acc, item) => {
                acc[item._id || 'unknown'] = item.count;
                return acc;
            }, {});

            const reviewTrends = results.reviewTrends.map(item => ({
                date: item._id,
                count: item.count,
                averageRating: Math.round(item.averageRating * 100) / 100,
            }));

            const topKeywords = results.topKeywords.map(item => ({
                keyword: item._id,
                count: item.count,
            }));
    ...
    async getMerchantEstablishments(merchantId: string): Promise<EstablishmentDocument[]> {
        try {
            ...
            this.appLogger.error('Failed to fetch establishments', error instanceof Error ? error.stack : String(error), 'ReviewService');
    ...
    async getMerchantReviews(
        ...
        try {
            ...
            if (
                ...
                (pipeline[0] as PipelineStage.Match).$match.establishmentId = { $in: objectIds };
            }
     ...
     */
    async getEstablishmentReviewSummary(establishmentId: string): Promise<any> {
        try {
            const pipeline: PipelineStage[] = [
                {
                    $match: {
            ...
            const ratingCounts = result.ratingBreakdown.reduce((acc, rating) => {
                acc[rating] = (acc[rating] || 0) + 1;
                return acc;
            }, {});

            // Process sentiment breakdown
            const sentimentCounts = result.sentimentBreakdown.reduce((acc, sentiment) => {
                if (sentiment) {acc[sentiment] = (acc[sentiment] || 0) + 1;}
                return acc;
            }, {});

            // Calculate average detailed ratings
            const detailedRatingsSum = result.averageDetailedRatings.reduce((acc, ratings) => {
                if (ratings) {
            ...
            const averageDetailedRatings = Object.keys(detailedRatingsSum).reduce((acc, key) => {
                acc[key] = Math.round((detailedRatingsSum[key].sum / detailedRatingsSum[key].count) * 100) / 100;
                return acc;
            }, {});
            ...
            return {
                ...
                responseRate: result.totalReviews > 0 ? Math.round((result.totalResponses / result.totalReviews) * 100) : 0,
                averageHelpfulness: result.totalReviews > 0 ? Math.round((result.totalHelpfulVotes / result.totalReviews) * 100) / 100 : 0
            };
    ...
    async getUserReviewStats(userId: string): Promise<any> {
        try {
            ...
            const monthlyActivity = result.monthlyReviews.reduce((acc, month) => {
            ...
            return {
                ...
                averageHelpfulness: result.totalReviews > 0 ? Math.round((result.totalHelpfulVotes / result.totalReviews) * 100) / 100 : 0,
                verificationRate: result.totalReviews > 0 ? Math.round((result.verifiedReviews / result.totalReviews) * 100) : 0,
    ...
    private findExistingReview(
        ...
        return this.reviewModel.findOne(query).session(session || null);
    }

    private analyzeReviewContent(comment: string): any {
        try {
            ...
            const negativeWords = ['terrible', 'awful', 'bad', 'horrible', 'disgusting', 'hate', 'worst', 'disappointing'];
    ...
    private detectInappropriateContent(comment: string): boolean {
        ...
        const inappropriateWords = [
            ...
            'offensive', 'inappropriate', // placeholder words
        ];
     ...
     */
    private detectFakeReview(_reviewData: any): boolean {
