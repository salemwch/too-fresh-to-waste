...
export class CreateReviewDto {
    ...
    @Transform(({ value }) => {
        ...
        if (typeof value === 'string') {return value.split(',').map(t => t.toLowerCase().trim());}
        return [];
    })
    tags?: string[];
...
export class ReviewInteractionDto {
    ...
    @IsIn(['helpful', 'not_helpful'], { message: 'Interaction type must be either "helpful" or "not_helpful"' })
