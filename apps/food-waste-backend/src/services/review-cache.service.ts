...
export class ReviewCacheService implements OnModuleInit, OnModuleDestroy {
    ...
    async getCachedReview(id: string): Promise<CachedReview | null> {
        try {
            ...
            if (redisClient) {
                cachedData = (await redisClient.get(key)) as string | null;
            } else {
    ...
    async invalidateEstablishmentReviews(establishmentId: string): Promise<void> {
        try {
            ...
            this.logger.error(`Failed to invalidate establishment reviews for ${establishmentId}:`, error);
        }
    }
    ...
    async warmUpCache(reviews: ReviewDocument[]): Promise<void> {
        try {
            const cachePromises = reviews.map(review =>
                this.cacheReview(review, this.DEFAULT_TTL * 2) // Longer TTL for warmed cache
            );
    ...
    private async updateCacheIndexes(review: ReviewDocument): Promise<void> {
        try {
            ...
            if (redisClient) {
                ...
                this.logger.debug(`Updated cache indexes for review ${reviewId}: establishment=${establishmentId}, user=${reviewerId}`);
            } else {
            ...
            if (redisClient) {
                ...
                if (cachedReview && typeof cachedReview === 'string') {
                    try {
                        ...
                        this.logger.warn(`Failed to parse cached review for view count update: ${reviewId}`, parseError);
                    }
                }
    ...
    private startCleanupInterval(): void {
        ...
        this.cleanupInterval = setInterval(() => {
            if (!this.redisService.isConnected()) { // Redis handles TTL automatically
