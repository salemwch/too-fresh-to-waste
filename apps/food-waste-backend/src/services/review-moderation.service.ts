...
export class ReviewModerationService {
    ...
    checkForProfanity(content: string): boolean {
        try {
            ...
            const hasSpamPatterns = this.spamIndicators.some(pattern =>
            ...
            if (containsInappropriateContent) {
                this.logger.warn(`Content flagged for inappropriate language: ${content.substring(0, 50)}...`);
            }
    ...
    validateContent(content: string): boolean {
        try {
            ...
            if (!isValid) {
                this.logger.warn(`Content failed validation: ${JSON.stringify(validations)} - Content: ${content.substring(0, 50)}...`);
            }
    ...
    private applyContentChecks(
        ...
        flags: string[],
        update: (status: 'APPROVED' | 'REJECTED' | 'PENDING', reason: string, confidence: number) => void
    ) {
        let status: 'APPROVED' | 'REJECTED' | 'PENDING' = 'APPROVED';
        let reason = '';
        let confidence = 0.8;
    ...
    private isSuspiciousRating(rating: number, content: string): boolean {
        const positiveWords = ['great', 'excellent', 'amazing', 'wonderful', 'love', 'perfect', 'awesome', 'fantastic'];
        const negativeWords = ['terrible', 'awful', 'bad', 'horrible', 'hate', 'worst', 'disgusting', 'disappointing'];
    ...
    private validateImages(images: any[]): { isValid: boolean; reason?: string } {
        try {
            if (images.length > 10) {
            ...
            for (const image of images) {
                ...
                if (image.size && image.size > 10 * 1024 * 1024) { // 10MB limit
