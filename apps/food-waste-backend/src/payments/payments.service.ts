...
export class PaymentService {
  ...
  async createPayment(
    ...
    try {
      ...
      const paymentCurrency = createPaymentDto.currency ?? Currency.TND;
      ...
      const payment = new this.paymentModel({
        ...
        cardInfo: {
          maskedCardNumber: this.smtPaymentService.maskCardNumber(
            createPaymentDto.cardDetails.cardNumber,
          ),
          cardType,
      ...
      try {
        ...
        savedPayment.smtResponse = {
          ...
          authorizationCode: smtResponse.authorizationCode ?? undefined,
          rrn: smtResponse.rrn ?? undefined,
          timestamp: new Date(),
        };
        ...
        this.appLogger.log(
          `CreatePayment DTO: ${JSON.stringify(createPaymentDto)}`,
          'PaymentsService',
        );
        ...
        this.appLogger.error(
          `SMT payment processing failed: ${(smtError as Error).message}`,
          'PaymentsService',
        );
        savedPayment.status = PaymentStatus.FAILED;
      ...
      this.appLogger.error(
        `Payment creation error: ${(error as Error).message}`,
        'PaymentsService',
      );
      if (
  ...
  async processRefund(
    ...
    try {
      if (!processRefundDto.transactionId) {
      ...
      this.appLogger.error(
        `Refund processing error: ${(error as Error).message}`,
        'PaymentsService',
      );
      throw error;
    }
  ...
  async handleWebhook(
    ...
    try {
      ...
      if (!isValid) {
        ...
        await webhook.save();
        this.appLogger.warn(
          `Invalid webhook signature for transaction: ${webhookPayload.transactionId}`,
          'PaymentsService',
        );
        return;
      }
      ...
      if (!payment) {
        ...
        await webhook.save();
        this.appLogger.warn(
          `Payment not found for webhook: ${webhookPayload.transactionId}`,
          'PaymentsService',
        );
        return;
      }
      ...
      if (newStatus !== null && newStatus !== undefined && newStatus !== previousStatus) {
        ...
        await payment.save();
        this.appLogger.log(
          `Payment status updated via webhook: ${payment.transactionId} - ${previousStatus} -> ${newStatus}`,
          'PaymentsService',
        );
      }
      ...
      try {
        ...
        this.appLogger.error(
          `Failed to update webhook status: ${(updateError as Error).message}`,
          'PaymentsService',
        );
      }
    }
  ...
  async retryFailedWebhooks(): Promise<number> {
    const failedWebhooks = await this.webhookModel
      .find({
        status: WebhookStatus.FAILED,
        retryCount: { $lt: 3 },
    ...
    for (const webhook of failedWebhooks) {
      try {
        ...
        this.appLogger.error(
          `Webhook retry failed for ${webhook._id}: ${(err as Error).message}`,
          'PaymentsService',
        );
        webhook.retryCount += 1;
