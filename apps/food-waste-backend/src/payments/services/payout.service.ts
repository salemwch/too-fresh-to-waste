...
export class PayoutService {
    ...
    async createLedgerEntry(
        ...
        const ledgerEntry = new this.ledgerModel({
            ...
            merchantName: `${merchant.firstName || ''} ${merchant.lastName || ''}`.trim() || 'Unknown Merchant',
     ...
     */
    async aggregatePendingByMerchant(): Promise<Array<{
        _id: Types.ObjectId;
        merchantName: string;
    ...
    async processMerchantPayout(
        ...
        const merchantName = pendingEntries[0].merchantName;
        const merchantEmail = pendingEntries[0].merchantEmail;
    ...
    async retryFailedPayouts(batchId: string): Promise<PayoutResult[]> {
        ...
        for (const entry of failedEntries) {
            const result = await this.processMerchantPayout(
                entry._id.toString(),
                batchId,
            );
            results.push(result);
