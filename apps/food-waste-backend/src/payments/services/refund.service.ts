...
export class RefundService {
     ...
     * @returns Refund result
     */
    async processExpiredOrderRefund(
        orderId: string,
        session?: ClientSession,
    ): Promise<RefundResult> {
        const payment = session
    ...
    async scheduleRefundRetry(paymentId: string): Promise<void> {
        ...
        const nextDelay = this.retryConfig.retryDelays[payment.retryCount];
    ...
    async findPaymentByOrderId(orderId: string): Promise<PaymentDocument | null> {
        return this.paymentModel.findOne({ orderId: new Types.ObjectId(orderId) });
    }
}
