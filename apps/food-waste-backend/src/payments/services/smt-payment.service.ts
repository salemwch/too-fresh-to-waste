...
const createGCMCipher = (algorithm: string, key: Buffer, iv: Buffer): GCMCipher => {
    return crypto.createCipheriv(algorithm, key, iv) as GCMCipher;
};

const createGCMDecipher = (algorithm: string, key: Buffer, iv: Buffer): GCMDecipher => {
    return crypto.createDecipheriv(algorithm, key, iv) as GCMDecipher;
};
...
export class SMTPaymentService {
    ...
    constructor(
        ...
        this.config = {
            ...
            environment: this.configService.get<string>('SMT_ENVIRONMENT') as 'sandbox' | 'production' | 'development',
            timeout: this.configParserService.parseNumber('SMT_TIMEOUT', 30000),
            maxRetries: this.configParserService.parseNumber('SMT_MAX_RETRIES', 3),
            ...
            keyRotationInterval: this.configParserService.parseNumber('SMT_KEY_ROTATION_INTERVAL', 24 * 60 * 60 * 1000), // 24 hours
    ...
    private validateSecurityConfig(): void {
        if (!this.config.encryptionSalt || this.config.encryptionSalt.length < 32) {
            throw new Error('SMT_ENCRYPTION_SALT must be at least 32 characters long. Generate with: crypto.randomBytes(64).toString("hex")');
        }
        ...
        if (this.config.environment === 'production') {
            if (!this.config.masterEncryptionKey || this.config.masterEncryptionKey.length < 64) {
                throw new Error('SMT_MASTER_ENCRYPTION_KEY must be at least 64 characters long in production. Generate with: crypto.randomBytes(32).toString("hex")');
            }
    ...
    async processPayment(paymentRequest: SMTPaymentRequest): Promise<SMTPaymentResponse> {
        try {
            this.logger.log(`Processing SMT payment for transaction: ${paymentRequest.merchantTransactionId}`);
            ...
            const smtResponse: SMTPaymentResponse = {
                ...
                status: this.mapSMTStatusToPaymentStatus(response.data.status ?? 'pending'),
                responseCode: response.data.responseCode ?? 'N/A',
                responseMessage: response.data.responseMessage ?? `Payment ${response.data.status ?? 'unknown'}`,
                authorizationCode: response.data.authorizationCode,
                rrn: response.data.rrn,
                redirectUrl: response.data.redirectUrl,
                error: response.data.error,
                success: (response.data.status?.toLowerCase() === 'success' || response.data.status?.toLowerCase() === 'completed'),
            };


            const paymentStatus: PaymentStatus = this.mapSMTStatusToPaymentStatus(smtResponse.status);
    ...
    async processRefund(refundRequest: SMTRefundRequest): Promise<SMTPaymentResponse> {
        try {
            this.logger.log(`Processing SMT refund for transaction: ${refundRequest.originalTransactionId}`);
    ...
    private async encryptCardData(cardData: SecureCardData): Promise<string> {
        if (this.config.environment === 'development') {
            this.logger.warn('Using base64 encoding for card data in development mode - NEVER use in production');
     ...
     */
    private getOrCreateEncryptionKey(): Promise<EncryptionKeyPair> {
        ...
        return this.generateEncryptionKey(currentKeyId);
    }

    /**
     * Get encryption key by ID
     */
    private getEncryptionKey(keyId: string): Promise<EncryptionKeyPair | null> {
        const cachedKey = this.keyCache.get(keyId);
        if (cachedKey) {
            return Promise.resolve(cachedKey);
        }
        return this.loadEncryptionKey(keyId);
    }
    ...
    private async loadEncryptionKey(keyId: string): Promise<EncryptionKeyPair | null> {
        ...
        try {
            ...
            if (keyRecord.keyType === KeyType.HSM_REFERENCE) {
                return await this.loadHSMKey(keyRecord);
            }
            ...
            decryptedKeyMaterial = Buffer.concat([
                decipher.update(encrypted),
                decipher.final()
            ]);
     ...
     */
    private loadHSMKey(keyRecord: any): Promise<EncryptionKeyPair | null> {
        try {
            ...
            return Promise.resolve(null); // Implement actual HSM integration
    ...
    private async updateKeyAccessTracking(keyId: string): Promise<void> {
        try {
            await this.encryptionKeyModel.updateOne(
                {
                    keyId,
                    merchantId: this.config.merchantId,
                    status: KeyStatus.ACTIVE
    ...
    private async storeEncryptionKey(keyPair: EncryptionKeyPair, keyMaterial: Buffer): Promise<void> {
        ...
        try {
            ...
            encryptedKeyMaterial = Buffer.concat([
    ...
    getCardType(cardNumber: string): PaymentMethod {
        ...
        if (/^4/.test(cleanNumber)) { return PaymentMethod.VISA; }
        if (/^5[1-5]/.test(cleanNumber) || /^2[2-7]/.test(cleanNumber)) { return PaymentMethod.MASTERCARD; }
        if (/^9/.test(cleanNumber)) { return PaymentMethod.EDAHABIA; } // Edahabia cards typically start with 9
