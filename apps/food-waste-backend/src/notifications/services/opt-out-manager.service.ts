...
export class OptOutManagerService {
  ...
  constructor(
    ...
    this.defaultExpirationDays = this.configService.get<number>('sms.optOut.defaultExpirationDays', 365);
  ...
  async checkOptOutStatus(phoneNumber: string, includeAuditLog: boolean = false, includeStats: boolean = false): Promise<OptOutStatusResponseDto> {
    ...
    try {
      ...
      if (!this.phoneValidator.validatePhoneNumber(sanitized)) {
        this.logger.warn(`Invalid phone number format attempted: ${this.phoneValidator.maskPhoneNumber(phoneNumber)}`, {
          operationId,
  ...
  async handleOptOut(optOutRequest: OptOutRequestDto): Promise<OptOutOperationResponseDto> {
    ...
    try {
      ...
      const result = await this.optOutModel.findOneAndUpdate(
        { phoneNumber: sanitized },
        updateData,
        {
          upsert: true,
    ...
    try {
      ...
      const result = await this.optOutModel.findOneAndUpdate(
        { phoneNumber: sanitized },
        {
          ...
          userId: optInRequest.userId ? new Types.ObjectId(optInRequest.userId) : existingRecord.userId,
  ...
  private validateBulkRequestSize(phoneNumberCount: number): void {
    if (phoneNumberCount > this.maxBulkSize) {
      throw new BadRequestException(`Bulk request exceeds maximum size of ${this.maxBulkSize} phone numbers`);
    }
  }

  private async processPhoneNumbers(
    phoneNumbers: string[],
    operationId: string
  ): Promise<{
  ...
  private logPhoneNumberValidationError(operationId: string, phoneNumber: string, error: Error): void {
  ...
  private async fetchOptOutRecords(sanitizedNumbers: string[]): Promise<LeanDocument<OptOutRecordDocument>[]> {
    return await this.optOutModel
      .find({
  ...
  private createRecordLookupMap(records: LeanDocument<OptOutRecordDocument>[]): Map<string, LeanDocument<OptOutRecordDocument>> {
  ...
  private logBulkOperationSuccess(operationId: string, response: BulkOptOutStatusResponseDto): void {
  ...
  async bulkCheckOptOutStatus(bulkRequest: BulkOptOutCheckDto): Promise<BulkOptOutStatusResponseDto> {
    const operationId = this.generateOperationId();
    const startTime = Date.now();
    ...
    try {
      ...
      const { sanitizedNumbers, phoneNumberMap, results, totalErrors } = await this.processPhoneNumbers(
      ...
      this.logBulkOperationError(operationId, bulkRequest.phoneNumbers.length, error as Error, Date.now() - startTime);
  ...
  async cleanupExpiredRecords(): Promise<{ updated: number; errors: number }> {
    ...
    try {
      const result = await this.optOutModel.updateMany(
        {
  ...
  async canSendMessage(phoneNumber: string, messageType: string = 'general'): Promise<{ canSend: boolean; reason?: string; record?: OptOutStatusResponseDto }> {
    try {
      ...
      const record: OptOutRecordDocument | null = await this.optOutModel.findOne({
      ...
      return {
        canSend,
        reason: canSend ? undefined : `Phone number is opted out for ${messageType} messages (scope: ${record.scope})`,
        record: status
      };
    } catch (error) {
  ...
  async getOptOutStatistics(startDate?: Date, endDate?: Date): Promise<{
    totalOptOuts: number;
    totalOptIns: number;
    ...
    try {
      ...
      const [
        ...
        this.optOutModel.countDocuments({ ...dateFilter, isOptedOut: false, optedInAt: { $exists: true } }).exec(),
        this.optOutModel.countDocuments({ isOptedOut: true, status: OptOutRecordStatus.ACTIVE }).exec(),
        this.optOutModel.countDocuments({ status: OptOutRecordStatus.EXPIRED }).exec(),
        this.optOutModel.aggregate([
          { $match: dateFilter },
        ...
        this.optOutModel.countDocuments({
