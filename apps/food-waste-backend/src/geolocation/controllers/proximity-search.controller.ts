import { Controller, Post, Get, Body, Query, UseGuards, Logger, HttpCode, HttpStatus, ParseIntPipe } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiBearerAuth, ApiQuery } from '@nestjs/swagger';
import { JwtAuthGuard } from '../../auth/guards/jwt-auth.guard';
import { ProximitySearchService, ProximitySearchOptions } from '../services/proximity-search.service';
...
export class ProximitySearchController {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
   searchOffers(
    ...
    return  this.proximitySearchService.searchOffers(searchDto, options);
  }
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
   searchAll(
    @Body() searchRequest: ComprehensiveSearchDto
  ): Promise<{
    ...
    const options: ProximitySearchOptions = {
      ...
      establishmentTypes: searchRequest.options.establishmentTypes?.length ? searchRequest.options.establishmentTypes : undefined,
      offerCategories: searchRequest.options.offerCategories?.length ? searchRequest.options.offerCategories : undefined,
    ...
    return  this.proximitySearchService.searchAll(searchRequest.searchParams, options);
  }
  ...
  @ApiResponse({ status: 404, description: 'Offer not found' })
   getNearbyEstablishments(
    ...
    return  this.proximitySearchService.getNearbyEstablishments(
      offerId,
      radius ? +radius : 2000
    );
  }
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
   getEstablishmentsInDeliveryRadius(
    @Body() body: {
    ...
    return  this.proximitySearchService.getEstablishmentsInDeliveryRadius(
      userLocation,
  ...
  @ApiQuery({ name: 'categories', type: 'string', required: false, description: 'Comma-separated categories' })
  @ApiQuery({ name: 'onlyActive', type: 'boolean', required: false, description: 'Only active establishments' })
  @ApiResponse({
    status: 200,
