...
import { AdminAuditService, AuditLogResponse, AuditStatisticsResult } from '../services/admin-audit.service';
import { GetAnalyticsQueryDto, GetAuditLogsQueryDto } from '../dto/admin-analytics.dto';
import { PlatformAnalytics, AdminAction, AuditLogValue } from '../interfaces/admin-analytics.interface';
...
export class AdminAnalyticsController {
  ...
  @ApiQuery({ name: 'period', required: false, enum: ['day', 'week', 'month', 'quarter', 'year', 'custom'] })
  ...
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Max number of activities (default: 100)' })
  ...
  private mapToResponseDto(analytics: PlatformAnalytics): PlatformAnalyticsResponseDto {
    return {
      ...
      establishments: {
        ...
        topPerformingEstablishments: analytics.establishments.topPerformingEstablishments.map(perf => ({
          id: perf.id,
          name: perf.name,
          type: perf.type,
  ...
  private mapAuditLogsToResponseDto(auditLogs: AuditLogResponse): AuditLogResponseDto {
    return {
      logs: auditLogs.logs.map(log => ({
        ...
        action: log.action,
        targetType: log.targetType as 'user' | 'establishment' | 'order' | 'review' | 'offer' | 'system',
  ...
  private mapAuditStatsToResponseDto(statistics: AuditStatisticsResult): AuditStatisticsResponseDto {
    return {
  ...
  private mapRecentActivityToResponseDto(
    ...
    return {
      activities: (activities as Record<string, unknown>[]).map((activity: Record<string, unknown>) => ({
        ...
        targetType: activity['targetType'] as 'user' | 'establishment' | 'order' | 'review' | 'offer' | 'system',
        ...(activity['targetId'] !== undefined && { targetId: activity['targetId'] as string }),
        ...(activity['previousValue'] !== undefined && { previousValue: activity['previousValue'] as Record<string, AuditLogValue> }),
        ...(activity['newValue'] !== undefined && { newValue: activity['newValue'] as Record<string, AuditLogValue> }),
  ...
  private mapExportToResponseDto(
    ...
    if (format === 'csv') {
      ...
      const jsonData = (exportedData as Record<string, unknown>[]).map((log: Record<string, unknown>) => ({
        id: (log._id?.toString() || log.id) as string,
  ...
  private validateAnalyticsQuery(query: GetAnalyticsQueryDto): void {
    ...
    if (query.period && !['day', 'week', 'month', 'quarter', 'year', 'custom'].includes(query.period)) {
      throw new BadRequestException(
