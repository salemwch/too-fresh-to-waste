...
import { NotificationTrigger, NotificationPriority } from '../../notifications/types/notification.types';
...
function isEstablishmentAggregationResult(result: unknown): result is EstablishmentAggregationResult {
...
export class EstablishmentManagementService implements IEstablishmentManagementService {
  ...
  async getEstablishmentOverview(): Promise<IEstablishmentOverview> {
    try {
      ...
      const _establishmentsByType = this.formatGroupedResults(stats.establishmentsByType);
  ...
  async approveEstablishment(
    ...
    try {
      ...
      if (approveDto.approved) {
        ...
        establishment.rejectionReason = undefined;
      } else {
      ...
      await this.auditService.logEstablishmentAction({
        adminId,
        adminEmail,
        action: approveDto.approved ? AdminAction.ESTABLISHMENT_APPROVED : AdminAction.ESTABLISHMENT_REJECTED,
        establishmentId,
        previousValue,
        newValue: {
          status: establishment.status,
      ...
      this.logger.error(`Failed to ${approveDto.approved ? 'approve' : 'reject'} establishment ${establishmentId}:`, error);
      throw error;
    }
  }
  ...
  async getEstablishmentStats(
    ...
    try {
      ...
      const periodStart = statsDto.startDate ? new Date(statsDto.startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); // Default: 30 days ago
  ...
  async getEstablishmentActivity(establishmentId: string, _days: number = 30): Promise<LeanDocument<AdminAuditLogDocument>[]> {
    try {
  ...
  private async sendApprovalNotification(
    ...
    try {
      ...
      if (!isPopulatedOwner(owner)) {
        this.logger.error(`No valid owner information found for establishment ${establishment._id.toString()}`);
        return;
      }
      ...
      const notificationData: ISendNotificationRequest = {
        type: 'email',
        trigger: isApproved ? NotificationTrigger.ESTABLISHMENT_APPROVED : NotificationTrigger.ESTABLISHMENT_REJECTED,
        target: {
          userId: owner._id,
  ...
  private async sendStatusChangeNotification(
    ...
    try {
      ...
      const notificationData: ISendNotificationRequest = {
        ...
        payload: {
          title,
          body: updateDto.reason ? `${body} Please contact support if you have any questions.` : body,
          data: {
            establishmentId: establishment._id.toString(),
            establishmentName: establishment.name,
      ...
      if (result.success) {
        ...
        this.logger.error(
          `Failed to send status change notification: ${result.error}`
        );
      }
  ...
  async cancelScheduledReactivation(establishmentId: string): Promise<void> {
    try {
      ...
      if (jobRef?.jobId && jobRef.status === 'pending') {
        ...
        if (job) {
          await job.remove();
          this.logger.log(`Cancelled reactivation job ${jobRef.jobId} for establishment ${establishmentId}`);
        }
