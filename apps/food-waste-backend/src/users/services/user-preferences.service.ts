...
interface AuditLogDetails {
    ...
    changes?: Partial<UserPreferences | NotificationPreferences | PrivacyPreferences | DiscoveryPreferences>;
...
export class UserPreferencesService {
    ...
    async updatePrivacyPreferences(
        ...
        user.preferences.privacy = this.deepMerge(
    ...
    async updateDiscoveryPreferences(
        ...
        if (discoveryPrefs.maxDistance && (discoveryPrefs.maxDistance < 100 || discoveryPrefs.maxDistance > 50000)) {
            throw new BadRequestException('Max distance must be between 100m and 50km');
        }

        if (discoveryPrefs.minDiscount && (discoveryPrefs.minDiscount < 0 || discoveryPrefs.minDiscount > 100)) {
            throw new BadRequestException('Minimum discount must be between 0% and 100%');
        }

        // Update only discovery preferences
        user.preferences.discovery = this.deepMerge(
    ...
    async bulkUpdateNotificationPreferences(
        ...
        this.logger.log(`Bulk notification preferences update completed. Success: ${success}, Failed: ${failed}`);
