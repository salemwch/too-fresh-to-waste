...
export class SessionManagementService {
    ...
    async trustDevice(
        ...
        if (existingDevice) {
            ...
            if (activeTrustedDevices.length >= this.MAX_TRUSTED_DEVICES) {
                const oldestDevice = activeTrustedDevices
    ...
    async detectSuspiciousActivity(userId: string, deviceInfo: DeviceInfo): Promise<{
        ...
        if (currentLocation && recentLogins.length > 0) {
            const hasLocationHistory = recentLogins.some(
        ...
        if (recentLogins.length > 0) {
            ...
            if (hoursDiff < 2 && lastLogin.location && currentLocation &&
        ...
        if (isSuspicious) {
            this.logger.warn(`Suspicious activity detected for user: ${user.email}, Risk Score: ${riskScore}`);
        }
