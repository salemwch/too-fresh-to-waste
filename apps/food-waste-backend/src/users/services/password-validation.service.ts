...
export class PasswordValidationService {
    ...
    private readonly commonPasswords = new Set([
        ...
        'dragon', 'master', 'shadow', 'login', 'football',
        'iloveyou', 'superman', 'michael', 'ninja', 'mustang'
    ]);
    ...
    async validatePassword(
        ...
        this.logger.debug(`Password validation completed. Score: ${result.score}, Acceptable: ${result.isAcceptable}`);
        return result;
    }
    ...
    private checkUppercase(password: string, policy: PasswordPolicy, result: PasswordStrengthResult): void {
    ...
    private checkLowercase(password: string, policy: PasswordPolicy, result: PasswordStrengthResult): void {
    ...
    private checkNumbers(password: string, policy: PasswordPolicy, result: PasswordStrengthResult): void {
    ...
    private checkSpecialChars(password: string, policy: PasswordPolicy, result: PasswordStrengthResult): void {
    ...
    private validatePersonalInfo(
        password: string,
        userInfo: { email?: string; firstName?: string; lastName?: string; phoneNumber?: string } | undefined,
        policy: PasswordPolicy,
        result: PasswordStrengthResult
    ): void {
    ...
    private extractPersonalInfo(userInfo: { email?: string; firstName?: string; lastName?: string; phoneNumber?: string }): string[] {
        return [
    ...
    private buildUserInputsForZxcvbn(userInfo: { email?: string; firstName?: string; lastName?: string; phoneNumber?: string } | undefined): string[] {
        return userInfo ? [
            userInfo.email,
            userInfo.firstName,
            userInfo.lastName,
    ...
    private applyZxcvbnResults(zxcvbnResult: { score: number; crack_times_display: { offline_slow_hashing_1e4_per_second: string }; feedback: { warning?: string; suggestions: string[] } }, result: PasswordStrengthResult): void {
    ...
    private determineAcceptability(
        result: PasswordStrengthResult,
        policy: PasswordPolicy
    ): void {
        ...
        if (!scoreAcceptable) {
            result.feedback.push(`Password strength score (${result.score}/4) is below minimum required (${policy.minScore})`);
        }
    }
    ...
    async addPasswordToHistory(userId: string, hashedPassword: string): Promise<void> {
        ...
        if (user.securitySettings.passwordHistory.length > maxHistory) {
            user.securitySettings.passwordHistory = user.securitySettings.passwordHistory.slice(0, maxHistory);
        }
    ...
    async updatePasswordStrength(userId: string, passwordStrength: PasswordStrengthResult): Promise<void> {
        const user = await this.userModel.findById(userId);
        if (!user) {
            return;
        }
    ...
    async getPasswordSecurityReport(userId: string): Promise<{
        ...
        passwordAge: { needsChange: boolean; daysUntilExpiry: number; lastChanged?: Date };
        historyCount: number;
