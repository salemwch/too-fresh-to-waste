...
export class MfaService {
    ...
    async setupTotp(userId: string): Promise<MfaSetupResult> {
        ...
        const hashedBackupCodes = await Promise.all(
            backupCodes.map(async code => argon2.hash(code))
        );
        ...
        user.mfaSettings.methods = user.mfaSettings.methods.filter(
    ...
    async generateNewBackupCodes(userId: string): Promise<string[]> {
        ...
        const hashedBackupCodes = await Promise.all(
            newBackupCodes.map(async code => argon2.hash(code))
        );
    ...
    async requiresMfaForAction(userId: string, action: string): Promise<boolean> {
        ...
        return user.mfaSettings.requireForSensitiveActions &&
