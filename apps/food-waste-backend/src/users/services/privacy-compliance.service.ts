...
export class PrivacyComplianceService {
  ...
  private readonly encryptionKey = process.env.PRIVACY_ENCRYPTION_KEY || 'default-key-change-in-production';
  ...
  async validateTunisianCompliance(userId: string): Promise<{
    ...
    this.logger.log(`🇹🇳 Tunisia compliance check for user ${userId}: ${compliant ? 'COMPLIANT' : 'NON-COMPLIANT'}`);
  ...
  async withdrawConsent(
    userId: string,
    withdrawalData: ConsentWithdrawalDto
  ): Promise<void> {
    this.logger.log(`🔄 Processing consent withdrawal for user ${userId}, type: ${withdrawalData.consentType}`);
    ...
    const consentIndex = consentRecords.findIndex(record =>
      record.consentType === withdrawalData.consentType &&
  ...
  async exportUserData(
    ...
    const exportData: IUserDataExport = {
      ...
      applicationData: {
        ...
        notifications: exportRequest.includeApplicationData ? await this.getUserNotifications(userId) : []
      },
      exportMetadata: {
        ...
        version: '1.0',
        legal_notices: {
          tunisia_law: '🇹🇳 Data exported in compliance with Tunisia Law No. 2004-63 on Personal Data Protection',
  ...
  async processDataDeletion(
    ...
    this.logger.log(`🗑️ Processing data deletion for user ${userId}, type: ${deletionRequest.deletionType}`);
  ...
  private getTunisianDataCategories(consent: TunisianPrivacyConsentDto): string[] {
    ...
    if (consent.dataProcessingConsent) { categories.push('personal_identifiers', 'contact_information') };
    if (consent.locationTrackingConsent) { categories.push('location_data', 'geolocation_history') };
    if (consent.communicationConsent) { categories.push('communication_preferences', 'marketing_data') };
  ...
  private getInternationalDataCategories(consent: InternationalPrivacyConsentDto): string[] {
    ...
    if (consent.analyticsOptIn) { categories.push('usage_analytics', 'behavioral_data') };
    if (consent.profilingOptIn) { categories.push('user_preferences', 'recommendation_data') };
    if (consent.cookiesConsent) { categories.push('cookie_data', 'session_information') };
  ...
  private async restrictDataProcessing(userId: string, consentType: ConsentType): Promise<void> {
    ...
    this.logger.log(`🚫 Restricting data processing for user ${userId}, consent type: ${consentType}`);
  ...
  async getSystemComplianceOverview(): Promise<ISystemComplianceOverview> {
    ...
    try {
      ...
      const tunisiaComplianceRate = totalUsersCount > 0
        ? Math.round((tunisiaComplianceData.compliantUsers / totalUsersCount) * 100 * 100) / 100
        : 0;

      const internationalComplianceRate = totalUsersCount > 0
        ? Math.round((internationalComplianceData.gdprCompliant / totalUsersCount) * 100 * 100) / 100
        : 0;
      ...
      this.logger.log(`✅ Compliance overview generated successfully. Overall compliance: ${auditSummary.complianceScore}`);
      return overview;
    } catch (error) {
  ...
  private async getTunisiaComplianceStats(): Promise<{
    ...
    const pendingConsents = missingDataProcessingConsent + missingLocationConsent + missingCommunicationConsent;
  ...
  private async getConsentRecordsStats(): Promise<{ totalRecords: number }> {
    const result = await this.userModel.aggregate([
      ...
      { $project: { consentRecordsCount: { $size: { $ifNull: ['$privacySettings.consentRecords', []] } } } },
  ...
  private generateAuditSummary(
    tunisiaData: { compliantUsers: number; pendingConsents: number; missingDataProcessingConsent: number },
