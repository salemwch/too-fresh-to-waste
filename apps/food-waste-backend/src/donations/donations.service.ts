...
import {
    ...
    DonationBadge,
} from './schemas/user-donation.schema';
import {
    DonationStatsResponseDto,
    UserDonationStatsResponseDto,
} from './dto/donation-stats.dto';
import {
    CreateDonationInput,
    DONATION_CONSTANTS,
    BADGE_THRESHOLDS,
...
export class DonationsService {
    ...
    async createDonation(input: CreateDonationInput): Promise<UserDonationDocument> {
        try {
            ...
            const donation = new this.userDonationModel({
                ...
                amount: input.amount,
                currency: input.currency || DONATION_CONSTANTS.DEFAULT_CURRENCY,
    ...
    private async checkPoolTargetReached(poolId: Types.ObjectId): Promise<void> {
        ...
        if (pool && pool.currentAmount >= pool.targetAmount && pool.status === DonationPoolStatus.ACTIVE) {
