...
export interface SecureCookieOptions {
    ...
    secure?: boolean;        // HTTPS only (MITM protection)
    sameSite?: 'strict' | 'lax' | 'none';  // CSRF protection
    maxAge?: number;         // milliseconds (session lifetime)
    path?: string;           // URL scope restriction
    domain?: string;         // Domain scope restriction
    signed?: boolean;        // Cookie integrity verification
}
...
export class CookieSecurityUtil {
    ...
    static clearAuthCookies(res: Response, isProduction: boolean, domain?: string): void {
        ...
        // ✓ Clear access token (must match original path: /api)
        res.clearCookie('access_token', { ...baseOptions, path: '/api' });
     ...
     */
    static getSecurityConfig(isProduction: boolean, domain?: string): Record<string, any> {
        return {
            securityAttributes: {
