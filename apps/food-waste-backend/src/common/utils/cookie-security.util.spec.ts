...
import { CookieSecurityUtil, SecureCookieOptions } from './cookie-security.util';
...
describe('CookieSecurityUtil', () => {
    ...
    beforeEach(() => {
        cookieStore = {};
        const cookieFn = jest.fn((name: string, value: string, options: any) => {
            cookieStore[name] = { value, options };
            return mockResponse as Response;
        }) as any;

        const clearCookieFn = jest.fn((name: string, options: any) => {
    ...
    describe('setAccessTokenCookie', () => {
        ...
        it('should set access token cookie with path=/api', () => {
            CookieSecurityUtil.setAccessTokenCookie(
                mockResponse as Response,
                'test-access-token',
                true,
            ...
            const cookieOptions = (mockResponse.cookie as jest.Mock).mock.calls[0][2];
            expect(cookieOptions.path).toBe('/api');
        });
    ...
    describe('isValidCookieValue', () => {
        ...
        it('should return false for non-string value', () => {
            expect(CookieSecurityUtil.isValidCookieValue(null as any)).toBe(false);
            expect(CookieSecurityUtil.isValidCookieValue(undefined as any)).toBe(false);
        });
