...
export class RequestLoggingInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
        ...
        const correlationId = request.correlationId || request.headers['x-correlation-id'] as string || uuidv4();
     ...
     */
    private shouldLogResponseBody(request: Request, response: Response): boolean {
        ...
        if (contentLength && parseInt(contentLength, 10) > 100000) { // 100KB limit
            return false;
        }
