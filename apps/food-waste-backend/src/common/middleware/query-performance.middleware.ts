...
export class QueryPerformanceService {
    ...
    constructor(
        ...
        this.SLOW_QUERY_THRESHOLD_MS = parseInt(process.env.SLOW_QUERY_THRESHOLD_MS) || 1000;
    }
    ...
    setupMonitoring(): void {
        if (process.env.NODE_ENV === 'production' && process.env.ENABLE_QUERY_MONITORING !== 'true') {
            this.logger.log('Query monitoring disabled in production (set ENABLE_QUERY_MONITORING=true to enable)');
            return;
        }
        ...
        const methodPattern = /^(find|findOne|findOneAndUpdate|findOneAndDelete|updateOne|updateMany|deleteOne|deleteMany|countDocuments|aggregate)$/;

        // Monitor Mongoose queries
        this.connection.plugin((schema) => {
            // Pre-hook to capture start time
            schema.pre(methodPattern, function (this: any) {
                this._startTime = Date.now();
            });

            // Post-hook to measure execution time
            schema.post(methodPattern, function (this: any, _result: unknown) {
                const duration = Date.now() - (this._startTime || Date.now());
                const operation = this.op || this.constructor?.name || 'unknown';
                ...
                if (duration >= service.SLOW_QUERY_THRESHOLD_MS) {
                    service.logSlowQuery(
                        ...
                        collection,
                        duration,
                        typeof this.getQuery === 'function' ? this.getQuery() : undefined,
     ...
     * Note: Internal method exposed for middleware callback access
     */
    logSlowQuery(operation: string, collection: string, duration: number, query: unknown, options: unknown): void {
    ...
    private logQueryStats(): void {
        ...
        this.logger.log(`   Slow Queries (>${this.SLOW_QUERY_THRESHOLD_MS}ms): ${this.slowQueries.length}`);
