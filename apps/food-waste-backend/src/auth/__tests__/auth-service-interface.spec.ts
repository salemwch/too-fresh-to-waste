...
import type { IEmailService } from '../../email/interfaces';
import type { IUsersService } from '../../users/interfaces';
import type {
...
describe('AuthService with Interface-Based Injection', () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<IPasswordPolicyService>;
  ...
  describe('Password Validation with Interface Mocking', () => {
    it('should validate password using IPasswordPolicyService', () => {
      ...
      mockPasswordPolicyService.validatePassword.mockReturnValue(weakPasswordResult);
      ...
      expect(mockPasswordPolicyService.validatePassword).toHaveBeenCalledWith('weak', {
        email: 'test@example.com',
      });
    });
    ...
    it('should accept strong password', () => {
      ...
      mockPasswordPolicyService.validatePassword.mockReturnValue(strongPasswordResult);

      // Act
      const result = mockPasswordPolicyService.validatePassword('StrongP@ssw0rd!2024', {
        email: 'test@example.com',
      });
  ...
  describe('User Registration Flow with Interface Mocking', () => {
    it('should register user and send verification email', async () => {
      ...
      const mockUser = {
        ...
        role: 'consumer',
      } as unknown as Parameters<typeof mockEmailService.sendVerificationEmail>[0];

      mockUsersService.findByEmail.mockResolvedValue(
        null as unknown as Awaited<ReturnType<typeof mockUsersService.findByEmail>>,
      ); // No existing user
      mockUsersService.create.mockResolvedValue(
        mockUser as unknown as Awaited<ReturnType<typeof mockUsersService.create>>,
      );
    ...
    it('should prevent duplicate email registration', async () => {
      // Arrange
      const existingUser = { email: 'existing@example.com' } as unknown as Awaited<
        ReturnType<typeof mockUsersService.findByEmail>
      >;
  ...
  describe('Email Service Interface Mocking', () => {
    it('should send welcome email successfully', async () => {
      ...
      const mockUser = {
        ...
        firstName: 'John',
      } as unknown as Parameters<typeof mockEmailService.sendWelcomeEmail>[0];
    ...
    it('should handle email sending failure gracefully', async () => {
      // Arrange
      const mockUser = { email: 'user@example.com' } as unknown as Parameters<
        typeof mockEmailService.sendVerificationEmail
      >[0];
      mockEmailService.sendVerificationEmail.mockResolvedValue(false);

      // Act
      const result = await mockEmailService.sendVerificationEmail(mockUser, 'token');
