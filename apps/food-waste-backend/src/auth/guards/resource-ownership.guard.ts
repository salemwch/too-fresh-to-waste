...
export class ResourceOwnershipGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const config = this.reflector.getAllAndOverride<OwnershipCheckConfig>(
            CHECK_OWNERSHIP_KEY,
        ...
        if (!resource) {
            throw new NotFoundException(
                `${config.resourceType} with ID '${resourceId}' not found`,
            );
        }
    ...
    private async fetchResource(resourceType: string, resourceId: string): Promise<any> {
        try {
            ...
            this.logger.error(
                `Failed to fetch ${resourceType} ${resourceId}`,
                error,
            );
            return null;
        }
