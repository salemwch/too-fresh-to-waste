...
export class TokenService {
  ...
  async validateRefreshToken(
    ...
    try {
      ...
      const jwtRefreshSecretToken = this.configService.get<string>('JWT_REFRESH_SECRET');
      const payload = await this.jwtService.verifyAsync<TokenPayload>(token, {
        ...(jwtRefreshSecretToken !== undefined ? { secret: jwtRefreshSecretToken } : {}),
      });
