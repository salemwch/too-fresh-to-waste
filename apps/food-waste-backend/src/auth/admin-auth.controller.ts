...
export class AdminAuthController {
  ...
  @ApiOperation({
    ...
    description:
      'Admin endpoint to unlock user accounts that have been locked due to failed login attempts',
  })
  ...
  @ApiOperation({
    summary: 'Get list of locked accounts',
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Admin access required' })
  @Get('locked-accounts')
  async getLockedAccounts(@Query('page') page: number = 1, @Query('limit') limit: number = 20) {
    ...
    const totalLocked = await this.usersService['userModel'].countDocuments({
      deletedAt: null,
      accountLockedUntil: { $gt: currentDate },
    });
  ...
  @ApiOperation({
    summary: 'Get authentication security statistics',
    description:
      'Retrieve authentication statistics including successful/failed logins within a date range',
  })
  @ApiQuery({
    name: 'fromDate',
    required: false,
  ...
  @ApiOperation({
    summary: 'Clear all IP blocks and login attempts',
    description:
      'Admin endpoint to clear all IP blocks and failed login attempt counters from Redis',
  })
  ...
  @ApiOperation({
    summary: 'Get failed login attempts for a user',
      ...
      summary: {
        ...
        successRate:
          stats.successfulLogins && stats.failedLogins
            ? (
                (stats.successfulLogins / (stats.successfulLogins + stats.failedLogins)) *
                100
              ).toFixed(2)
            : '0.00',
      },
