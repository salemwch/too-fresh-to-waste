...
async function connectToDatabase(): Promise<typeof mongoose> {
    ...
    logger.log(`Connecting to MongoDB: ${mongoUri.replace(/\/\/[^:]+:[^@]+@/, '//***:***@')}`, 'INFO');
...
async function migratePhoneNumbers(mongooseInstance: typeof mongoose): Promise<MigrationSummary> {
    ...
    try {
        ...
        if (!db) {
            throw new Error('Failed to access MongoDB database. Connection state: ' + mongooseInstance.connection.readyState);
        }
        ...
        const usersWithPhone = await usersCollection.find({
        ...
        for (let i = 0; i < usersWithPhone.length; i += CONFIG.batchSize) {
            ...
            logger.log(`Processing batch ${Math.floor(i / CONFIG.batchSize) + 1} (${batch.length} users)`, 'INFO');
...
function printSummary(summary: MigrationSummary): void {
    ...
    logger.log('=========================================', 'INFO');
    logger.log(`Mode: ${CONFIG.dryRun ? 'DRY RUN (no changes applied)' : 'EXECUTE (changes applied)'}`, 'INFO');
    ...
    logger.log(`Failed updates: ${summary.failedUpdates}`, summary.failedUpdates > 0 ? 'WARNING' : 'INFO');
