...
async function verifySchemaIndexes(
    ...
    const status =
        indexes.length >= expectedMin ? 'PASS' :
        indexes.length >= expectedMin * 0.8 ? 'WARNING' :
        'FAIL';
...
function printIndexDetails(result: SchemaIndexCount): void {
    ...
    console.log(`   Expected: ≥${result.expectedMin} | Actual: ${result.actual} | Status: ${result.status}`);
