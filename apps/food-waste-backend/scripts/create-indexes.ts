...
async function createIndexesForCollection(
    ...
    for (const indexDef of indexes) {
        ...
        try {
            ...
            const timeTaken = Date.now() - startTime;
            console.error(`   ❌ ${indexName} - Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
async function main() {
    ...
    try {
        ...
        console.log('3. Review index usage after 1 week: db.collection.aggregate([{ $indexStats: {} }])');
