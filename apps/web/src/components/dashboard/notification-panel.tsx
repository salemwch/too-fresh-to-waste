  ...
  return (
    <div ref={panelRef} className='relative'>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className='absolute -top-0.5 -right-0.5 min-w-[14px] h-3.5 rounded-full bg-red-500 text-white text-[8px] font-bold flex items-center justify-center px-[2px] leading-none pointer-events-none'>
      ...
      {/* Dropdown panel */}
      {open && (
        <div className='absolute right-0 top-full mt-2 w-80 rounded-xl bg-white border border-slate-200 shadow-xl z-50 overflow-hidden'>
          {/* Panel header */}
          <div className='flex items-center justify-between px-3 py-2.5 border-b border-slate-100'>
            ...
            {unreadCount > 0 && (
              <button
                onClick={markAllRead}
                className='flex items-center gap-0.5 text-[11px] text-primary-600 hover:text-primary-700 font-medium transition-colors'
              >
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <button
                  ...
                  className={cn(
                    'w-full text-left flex items-start gap-3 px-3 py-2.5 transition-colors hover:bg-slate-50',
