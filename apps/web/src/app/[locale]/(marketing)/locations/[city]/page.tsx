  ...
  return (
    <>
      ...
      <main role='main'>
        ...
        <section className='bg-primary-500 px-lg py-4xl md:py-6xl' aria-labelledby='city-heading'>
          <div className='mx-auto max-w-3xl text-center'>
            <p className='text-sm font-medium uppercase tracking-widest text-white/70'>
              Too Fresh To Waste · Tunisia
            </p>
            <h1 id='city-heading' className='mt-lg text-3xl font-bold text-white md:text-5xl'>
              {t(city.heading, loc)}
            </h1>
            <p className='mt-2xl text-md leading-relaxed text-white/90 md:text-lg'>
              {t(city.intro, loc)}
            </p>
          </div>
        </section>
        ...
        {/* Local context - the unique-value block that keeps this off the
            doorway-page pile */}
        <section className='px-lg py-3xl md:py-4xl' aria-labelledby='context-heading'>
          <div className='mx-auto max-w-3xl'>
            <h2 id='context-heading' className='text-2xl font-semibold md:text-3xl'>
              Why food waste matters in {cityName}
            </h2>
            <p className='mt-lg text-base leading-relaxed text-muted-foreground'>
              {t(city.localContext, loc)}
            </p>

            <h3 className='mt-6xl text-xl font-semibold'>What you will find in a bag</h3>
        ...
        <section className='bg-muted px-lg py-3xl md:py-4xl' aria-labelledby='areas-heading'>
          <div className='mx-auto max-w-3xl'>
            <h2 id='areas-heading' className='text-2xl font-semibold md:text-3xl'>
              Areas we cover in {cityName}
            </h2>
            <ul className='mt-2xl flex flex-wrap gap-sm'>
              {city.neighborhoods.map(area => (
                <li
                  key={area}
                  className='rounded-full border border-border bg-card px-lg py-sm text-sm'
                >
                  {area}
                </li>
              ))}
        ...
        {faqItems.length > 0 && (
          <section className='px-lg py-3xl md:py-4xl' aria-labelledby='faq-heading'>
            <div className='mx-auto max-w-3xl'>
              <h2 id='faq-heading' className='text-2xl font-semibold md:text-3xl'>
                Frequently asked questions about {cityName}
              </h2>
              <dl className='mt-4xl space-y-2xl'>
                {faqItems.map(item => (
                  <div key={item.question} className='border-b border-border pb-2xl'>
                    <dt className='text-md font-semibold'>{item.question}</dt>
                    <dd className='mt-sm text-base leading-relaxed text-muted-foreground'>
                      {item.answer}
                    </dd>
                  </div>
                ))}
        ...
        {nearbyCities.length > 0 && (
          <section className='bg-muted px-lg py-3xl md:py-4xl' aria-labelledby='nearby-heading'>
            <div className='mx-auto max-w-3xl'>
                ...
                {nearbyCities.map(nearby => (
                  <li key={nearby.slug}>
                    <Link
                      ...
                      className='block rounded border border-border bg-card p-lg transition-colors hover:border-primary-500'
                    >
                      <span className='text-md font-semibold'>{t(nearby.name, loc)}</span>
                      <span className='mt-xs block text-sm text-muted-foreground'>
                        {t(nearby.region, loc)}
                      </span>
                    </Link>
                  </li>
