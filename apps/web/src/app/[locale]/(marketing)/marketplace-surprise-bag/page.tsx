...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'marketplaceBag' });
...
/** Paired by index with `howItWorks.steps`. */
const STEP_COLORS = ['bg-primary-500', 'bg-brand-green', 'bg-brand-teal'] as const;
...
/**
 * Paired by index with `what.categories` in the messages. Icons are not copy,
 * so they stay here; the locale test pins both lists at six.
 */
...
export default async function MarketplaceSurpriseBagPage({ params }: PageProps) {
  ...
  const whatBullets = t.raw('what.bullets') as string[];
  const categories = t.raw('what.categories') as Category[];
          ...
          <div className='mx-auto max-w-7xl px-2xl lg:px-4xl'>
            <div className='text-center mb-3xl'>
              ...
              <h2 className='font-heading text-4xl lg:text-5xl font-bold text-primary-500'>
                ...
                <span className='text-brand-green italic'>{t('benefits.titleEm')}</span>.
              </h2>
            </div>
          ...
          <div className='mx-auto max-w-7xl px-2xl lg:px-4xl'>
            <div className='flex flex-col md:flex-row items-center justify-between gap-2xl'>
              <div className='flex items-center gap-lg'>
                ...
                <div>
                  <p className='font-bold text-primary-500 text-base'>{t('urgency.title')}</p>
                  <p className='text-sm text-primary-500/75'>{t('urgency.body')}</p>
                </div>
              </div>
              <Link
                href='/merchant-signup'
