...
describe('Badge', () => {
  ...
  it('renders with custom text color', () => {
    ...
    expect(text.props['style']).toMatchObject(expect.objectContaining({ color: '#FFFFFF' }));
  });
  ...
  it('applies custom text style', () => {
    ...
    expect(text.props['style']).toMatchObject(expect.objectContaining({ fontSize: 20 }));
  });
  ...
  it('uses label as default accessibility label', () => {
    const { getByLabelText } = renderWithTheme(<Badge label='Default Label' />);
    expect(getByLabelText('Default Label')).toBeTruthy();
  });

  it('converts number label to string for accessibility', () => {
    const { getByLabelText } = renderWithTheme(<Badge label={42} />);
